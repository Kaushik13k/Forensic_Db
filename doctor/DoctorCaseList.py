...
from DoctorEvidanceUpdate import DocUpdate
from admin.global_variables import GlobalStatic, DbConnector


class DocCaseList(object):
    def __init__(self):
        super().__init__()

        self.__all_doccases_list = []
...
#         self.dclEmail_4.setObjectName("dclEmail_4")
        ...
        self.dclUpdate.clicked.connect(self.DocUpdateEvi)
        self.dvlDelete.clicked.connect(self.delete_data)
        self.dCl_search.clicked.connect(self.search_data)


    def DocUpdateEvi(self):
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label.setText(_translate("MainWindow", "DOCTOR CASE LIST"))
        ...
        self.dcl_Emanil_2.setText(_translate("MainWindow", "  Police"))
    ...
    def CaseLoadData(self):
            ...
            cur = conn.cursor()
            cur.execute("select c_id, c_name,c_typr,c_date,c_time,c_location,p_id,pst_id from case_ where d_id=?;",
                        (GlobalStatic.d_id))
            rows = cur.fetchall()
            self.tableWidget.setRowCount(50)
            tableindex = 0
            self.__all_doccases_list = rows
    ...
    def _cell_clicked(self, row_index, column_index):
        ...
        row = self.__all_doccases_list[row_index]
        print(f"Case selected: {row}")

        GlobalStatic.set_evidence_doc(row)
    ...
    def search_data(self):
        ...
        query = f"SELECT c_id, c_name,c_typr,c_date,c_time,c_location,p_id,pst_id FROM case_ WHERE {self.dcl_comboBox.currentText()} LIKE '%{self.dcl_search_entry.text()}%'"
    ...
    def delete_data(self):
        ...
        cur = conn.cursor()
        cur.execute("Update case_ set d_id = NULL where c_id = ?", (GlobalStatic.evi_evidence_doc_tuple[0]))
        conn.commit()
        conn.close()
        msg.setIcon(QMessageBox.Information)
        msg.setText("You Have Declined the Case")
