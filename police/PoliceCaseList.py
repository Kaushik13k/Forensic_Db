...
from policeEvidanceUpdate import PoliceEvidance


class PoliceCaseList(object):
    def __init__(self):
        super().__init__()

        self.__all_polcases_list = []
    ...
    def setupUi(self, MainWindow):
        ...
        self.dclUpdate.clicked.connect(self.AddEvidancePage)

    def AddEvidancePage(self):
        self.MainWindow = QtWidgets.QMainWindow()
        self.ui = PoliceEvidance()
    ...
    def retranslateUi(self, MainWindow):
        ...
        # item.setText(_translate("MainWindow", "Doctor"))
        self.dcl_Emanil_2.setText(_translate("MainWindow", "  Doctor"))
        self.dcl_Emanil_3.setText(_translate("MainWindow", "  Hospital"))
    ...
    def CaseLoadData(self):
        ...
        cur = conn.cursor()
        cur.execute("select c_id, c_name,c_typr,c_date,c_time,c_location,d_id, h_id from case_ where p_id=?;",
                    (GlobalStatic.police_id))
        rows = cur.fetchall()
        self.tableWidget.setRowCount(50)
        tableindex = 0
        self.__all_polcases_list = rows
    ...
    def _cell_clicked(self, row_index, column_index):
        row = self.__all_polcases_list[row_index]
        print(f"Case selected: {row}")

        # GlobalStatic.evi_upd_info_tuple(row)

        GlobalStatic.set_evidence(row)
        ...
        # self.dcl_Emanil_4.setText(row[8])
    ...
    def search_data(self):
        ...
        query = f"SELECT c_id, c_name,c_typr,c_date,c_time,c_location,d_id, h_id FROM case_ WHERE {self.dcl_comboBox.currentText()} LIKE '%{self.dcl_search_entry.text()}%'"
    ...
    def delete_data(self):
        ...
        cur = conn.cursor()
        cur.execute("Update case_ set p_id = NULL where c_id = ?", (GlobalStatic.evi_upd_info_tuple[0]))
        conn.commit()
        conn.close()
