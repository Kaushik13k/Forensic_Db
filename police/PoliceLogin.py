...
from PoliceCaseList import PoliceCaseList
from admin.global_variables import GlobalStatic


class DocUiMainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.pushButton.clicked.connect(self.LoginDoc)

    def main_page(self):
        self.MainWindow = QtWidgets.QMainWindow()
        self.ui = PoliceCaseList()
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label.setText(_translate("MainWindow", "POLICE LOGIN"))
    ...
    def LoginDoc(self):
        ...
        if username == "" or password == "":
            ...
            try:
                ...
                cur = conn.cursor()
                cur.execute("select * from police where p_email=? and p_password=?",
                            (username, password))
                police_tuple = cur.fetchone()
                print(police_tuple)

                GlobalStatic.set_police_id(police_tuple[0])
                if police_tuple == None:
                    ...
                    # self.root.destroy()
                    self.main_page()
                    # import hospital_main_pg
                    # messagebox.showinfo("Success", "User", parent=self.root)
                conn.close()
